package org.selostudios.translator.voiceToText.presentation
...
class VoiceToTextViewModel(
    ...
    val state = _state.combine(parser.state) { state, voiceResult ->
        state.copy(
            ...
            recordError = if (state.canRecord) voiceResult.error else "Unable to record without permission",
            displayState = when {
        ...
        viewModelScope.launch {
            while (true) {
                if (state.value.displayState == DisplayState.SPEAKING) {
                    _state.update { state ->
                        state.copy(
                            //toFix consider making this drop a number of these to avoid having a massive list
                            powerRatios = state.powerRatios + parser.state.value.powerRatio
                        )
                    }
                }
                delay(50)
    ...
    private fun toggleRecording(languageCode: String) {
        _state.update { state ->
            state.copy(powerRatios = emptyList())
        }
        parser.cancel()
        if (state.value.displayState == DisplayState.SPEAKING) {
